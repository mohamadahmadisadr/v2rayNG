...
import dev.sadr.atlas.AngApplication
...
import dev.sadr.atlas.dto.SubscriptionUpdateResult
import dev.sadr.atlas.dto.TestServiceMessage
import dev.sadr.atlas.dto.entities.ServersCache
import dev.sadr.atlas.dto.entities.SubscriptionCache
import dev.sadr.atlas.extension.isComplexType
import dev.sadr.atlas.extension.matchesPattern
import dev.sadr.atlas.extension.toSpeedString
...
class MainViewModel @Inject constructor(
    ...
    private val _selectedProfileFlow = MutableStateFlow(
        mmkvManager.getSelectServer()?.let { mmkvManager.decodeServerConfig(it) }
    )
    val selectedProfileFlow = _selectedProfileFlow.asStateFlow()

    private val _connectedAtFlow = MutableStateFlow(0L)
    val connectedAtFlow = _connectedAtFlow.asStateFlow()
    ...
    private fun updateTrafficSpeeds(content: String?) {
        val parts = content?.split(',') ?: return
        if (parts.size != 2) return
        val up = parts[0].toLongOrNull() ?: return
        val down = parts[1].toLongOrNull() ?: return
        _upSpeedFlow.value = up.toSpeedString()
        _downSpeedFlow.value = down.toSpeedString()
    }

    private fun resetTrafficSpeeds() {
        _upSpeedFlow.value = "0 B/s"
        _downSpeedFlow.value = "0 B/s"
    }

    fun updateSelectedGuid() {
        var guid = mmkvManager.getSelectServer() ?: ""
        val profile = if (guid.isEmpty()) null else mmkvManager.decodeServerConfig(guid)
        if (guid.isNotEmpty() && profile == null) {
            // Selection points at a deleted profile — clear the stale reference
            mmkvManager.setSelectServer("")
            guid = ""
        }
        _selectedGuidFlow.value = guid
    ...
    fun getSubscriptions(context: Context): List<GroupMapItem> {
        ...
        // Manually imported configs (not belonging to any subscription)
        if (mmkvManager.decodeServerList(AppConfig.DEFAULT_SUBSCRIPTION_ID).isNotEmpty()
            || subscriptionId == AppConfig.DEFAULT_SUBSCRIPTION_ID
        ) {
            groups.add(
                GroupMapItem(
                    id = AppConfig.DEFAULT_SUBSCRIPTION_ID,
                    remarks = "My Configs"
                )
            )
        }
        ...
        subscriptions.forEach { sub ->
            groups.add(
                GroupMapItem(
                    id = sub.guid,
                    remarks = sub.subscription.remarks.ifBlank { "Subscription" }
                )
            )
        }

        // Free servers discovered by the auto-best pipeline
        ...
        // No "All" tab anymore: make sure the current filter targets a real group
        if (groups.none { it.id == subscriptionId }) {
            subscriptionIdChanged(groups.firstOrNull()?.id ?: "")
        }
            ...
            viewModelScope.launch(Dispatchers.Main) {
                when (key) {
                    AppConfig.MSG_STATE_RUNNING -> {
                        ...
                        _isLoadingFlow.value = false
                        if (_connectedAtFlow.value == 0L) {
                            _connectedAtFlow.value = System.currentTimeMillis()
                        }
                    }

                    AppConfig.MSG_TRAFFIC_UPDATE -> {
                        updateTrafficSpeeds(content)
                        if (!_isRunningFlow.value) _isRunningFlow.value = true
                        if (_connectedAtFlow.value == 0L) {
                            _connectedAtFlow.value = System.currentTimeMillis()
                        }
                    }
