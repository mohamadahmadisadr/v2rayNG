...
import dev.sadr.atlas.ui.MainActivity
...
object VpnNotificationManager {
    ...
    fun startSpeedNotification() {
        // The loop always runs while the core is up: it feeds the UI speed stats;
        // the notification text itself is still gated by PREF_SPEED_ENABLED.
    ...
    fun showNotification(currentConfig: ProfileItem?) {
        ...
        mBuilder = NotificationCompat.Builder(service, channelId)
            ...
            .setContentTitle(currentConfig?.remarks ?: "AtlasVPN")
    ...
    private fun createNotificationChannel(): String {
        val channelId = "atlasvpn_channel_id"
        val channelName = "AtlasVPN Service"
    ...
    private fun updateSpeedNotificationOnce(lastZeroSpeed: Boolean): Boolean {
        ...
        try {
            ...
            // Broadcast current speeds to the UI (Home screen statistics)
            getService()?.let { service ->
                val upBps = ((proxyUplink + directUplink) / sinceLastQueryInSeconds).toLong()
                val downBps = ((proxyDownlink + directDownlink) / sinceLastQueryInSeconds).toLong()
                MessageUtil.sendMsg2UI(service, AppConfig.MSG_TRAFFIC_UPDATE, "$upBps,$downBps")
            }
