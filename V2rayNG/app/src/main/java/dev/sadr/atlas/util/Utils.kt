...
import dev.sadr.atlas.AppConfig.LOOPBACK
...
object Utils {
    ...
    fun isValidSubUrl(value: String?): Boolean {
        ...
        try {
            // Accept any http(s) URL; plain-http subscriptions are the user's choice.
            if (URLUtil.isHttpsUrl(value) || URLUtil.isHttpUrl(value)) return true
        } catch (e: Exception) {
     ...
     */
    // This fork always bundles the Xray core; must not depend on the applicationId,
    // which was rebranded to dev.sadr.atlas.
