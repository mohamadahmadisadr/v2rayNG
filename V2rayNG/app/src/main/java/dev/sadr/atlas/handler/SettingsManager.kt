...
import dev.sadr.atlas.AppConfig.GEOIP_PRIVATE
import dev.sadr.atlas.AppConfig.GEOSITE_PRIVATE
import dev.sadr.atlas.AppConfig.TAG_DIRECT
import dev.sadr.atlas.AppConfig.VPN
import dev.sadr.atlas.dto.V2rayConfig
...
import dev.sadr.atlas.enums.Language
import dev.sadr.atlas.enums.RoutingType
import dev.sadr.atlas.enums.VpnInterfaceAddressConfig
import dev.sadr.atlas.handler.MmkvManager.decodeAllServerList
import dev.sadr.atlas.handler.MmkvManager.decodeServerConfig
import dev.sadr.atlas.handler.MmkvManager.decodeSubsList
import dev.sadr.atlas.handler.MmkvManager.decodeSubscription
import dev.sadr.atlas.handler.MmkvManager.encodeSubscription
import dev.sadr.atlas.handler.MmkvManager.removeSubscription
...
object SettingsManager {
    ...
    fun swapSubscriptions(fromPosition: Int, toPosition: Int) {
        ...
        // Positions come from the visible list, which hides the internal default group
        val visibleIndices = subsList.indices.filter { subsList[it] != DEFAULT_SUBSCRIPTION_ID }
        if (fromPosition !in visibleIndices.indices || toPosition !in visibleIndices.indices) return

        Collections.swap(subsList, visibleIndices[fromPosition], visibleIndices[toPosition])
    ...
    private fun ensureDefaultSettings() {
        ...
        if (!MmkvManager.containsSettingsKey(AppConfig.PREF_AUTO_PING_ON_START)) {
            MmkvManager.encodeSettings(AppConfig.PREF_AUTO_PING_ON_START, true)
        }
