...
import dev.sadr.atlas.fmt.Hysteria2Fmt
import dev.sadr.atlas.fmt.ShadowsocksFmt
import dev.sadr.atlas.fmt.SocksFmt
import dev.sadr.atlas.fmt.TrojanFmt
import dev.sadr.atlas.fmt.VlessFmt
import dev.sadr.atlas.fmt.VmessFmt
import dev.sadr.atlas.fmt.WireguardFmt
...
object AngConfigManager {
    ...
    fun importBatchConfig(server: String?, subid: String, append: Boolean): Pair<Int, Int> {
        ...
        if (countSub > 0) {
            // Surface how many configs the subscription(s) actually delivered
            count += updateConfigViaSubAll().configCount
        }
    ...
    fun updateConfigViaSub(it: SubscriptionCache): SubscriptionUpdateResult {
        try {
            ...
            if (count > 0) {
                // Auto-name the subscription from server metadata unless the user
                // already gave it a meaningful name.
                if (it.subscription.remarks.isBlank() || it.subscription.remarks == "import sub") {
                    extractSubscriptionName(responseHeaders, url)?.let { name ->
     ...
     */
    /**
     * Derives a human-readable subscription name from the server's response
     * metadata: content-disposition filename, then profile-title (optionally
     * base64-encoded), then the host name as a last resort.
     */
    private fun extractSubscriptionName(headers: Map<String, String>, url: String): String? {
        headers["content-disposition"]?.let { cd ->
            Regex("filename\\*?=\"?([^\";]+)\"?").find(cd)?.groupValues?.get(1)?.let { raw ->
                val name = raw.removePrefix("UTF-8''").trim()
                if (name.isNotBlank()) {
                    return runCatching { Utils.decodeURIComponent(name) }.getOrNull() ?: name
                }
            }
        }
        headers["profile-title"]?.let { rawTitle ->
            val title = if (rawTitle.startsWith("base64:")) {
                Utils.decode(rawTitle.removePrefix("base64:"))
            } else {
                rawTitle
            }.trim()
    ...
    private fun importUrlAsSubscription(url: String): Int {
        ...
        subscriptions.forEach {
            if (it.subscription.url == url) {
                // Already imported: treat as success so the caller refreshes it
