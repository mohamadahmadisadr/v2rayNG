...
import dev.sadr.atlas.AppConfig.DEFAULT_SUBSCRIPTION_ID
import dev.sadr.atlas.AppConfig.PREF_IS_BOOTED
import dev.sadr.atlas.AppConfig.PREF_ROUTING_RULESET
...
object MmkvManager {
    ...
    fun removeServer(guid: String) {
        ...
        // Remove the guid from every server list it may appear in; the profile's
        // subscriptionId can be stale, so relying on it alone leaves ghost entries.
        val allSubIds = (decodeSubsList()
                + DEFAULT_SUBSCRIPTION_ID
                + AppConfig.AUTO_BEST_SUBSCRIPTION_ID).distinct()
    ...
    fun removeAllServer(): Int {
        ...
        encodeServerList(mutableListOf(), DEFAULT_SUBSCRIPTION_ID)
        return count
    }
    ...
    fun decodeSubscriptions(): List<SubscriptionCache> {
        ...
        decodeSubsList().forEach { key ->
            // The internal default group (ungrouped servers) is not a real subscription;
            // it is surfaced separately as "My Configs" and must not appear in lists.
            if (key == DEFAULT_SUBSCRIPTION_ID) return@forEach
    ...
    fun decodeRoutingRulesets(): MutableList<RulesetItem>? {
        ...
        return migrateRulesetRemarks(rulesets)
    }

    // Rulesets persisted before the preset assets were translated may still carry
    // the original Chinese remarks; rewrite them to the English names once.
    private val legacyRulesetRemarks = mapOf(
        "阻断udp443" to "Block UDP 443",
        "绕过局域网IP" to "Bypass LAN IP",
        "绕过局域网域名" to "Bypass LAN domains",
        "最终代理" to "Final rule: proxy",
        "绕过bittorrent" to "Bypass BitTorrent",
        "代理Google" to "Proxy Google",
        "代理海外公共DNSIP" to "Proxy overseas public DNS IP",
        "代理海外公共DNS域名" to "Proxy overseas public DNS domains",
        "代理IP" to "Proxy blocked sites IP",
        "代理GFW" to "Proxy GFW domain list",
        "最终直连" to "Final rule: direct",
        "绕过中国公共DNSIP" to "Bypass China public DNS IP",
        "绕过中国公共DNS域名" to "Bypass China public DNS domains",
        "绕过中国IP" to "Bypass China IP",
        "绕过中国域名" to "Bypass China domains"
    )

    private fun migrateRulesetRemarks(rulesets: MutableList<RulesetItem>): MutableList<RulesetItem> {
        var changed = false
        rulesets.forEach { item ->
            legacyRulesetRemarks[item.remarks]?.let {
                item.remarks = it
                changed = true
            }
        }
        if (changed) {
            settingsStorage.encode(PREF_ROUTING_RULESET, JsonUtil.toJson(rulesets))
        }
        return rulesets
    }
