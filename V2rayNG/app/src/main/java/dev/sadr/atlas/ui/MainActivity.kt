...
import dev.sadr.atlas.enums.PermissionType
...
import dev.sadr.atlas.handler.AutoBestConfigManager
...
import dev.sadr.atlas.handler.SubscriptionUpdater
...
class MainActivity : HelperBaseActivity() {
    ...
    private val groupsState = MutableStateFlow<List<dev.sadr.atlas.dto.GroupMapItem>>(emptyList())
    ...
    private var actionSheetTarget = mutableStateOf<Triple<String, ProfileItem, Int>?>(null)
    private var showImportSheet = mutableStateOf(false)
    private var mainTabIndex = mutableIntStateOf(0)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            dev.sadr.atlas.ui.theme.V2Theme {
                ...
                MainScreen(
                    ...
                    onNavItemClick = { id -> onNavItemClick(id) },
                    onSelectServer = { guid -> setSelectServer(guid) },
                    onMoreClick = { guid, profile, position ->
                        actionSheetTarget.value = Triple(guid, profile, position)
                    },
                    onImportClick = { showImportSheet.value = true }
                )

                actionSheetTarget.value?.let { (guid, profile, position) ->
                    ServerActionSheet(
                        profile = profile,
                        onDismiss = { actionSheetTarget.value = null },
                        onAction = { action ->
                            actionSheetTarget.value = null
                            when (action) {
                                ServerAction.ShowQr -> qrCodeGuid.value = guid
                                ServerAction.CopyLink -> share2Clipboard(guid)
                                ServerAction.CopyFull -> shareFullContent(guid)
                                ServerAction.Edit -> editServer(guid, profile)
                                ServerAction.Delete -> removeServer(guid, position)
                            }
                        }
                    )
                }
        ...
        // One-time migration: auto-best used to be on by default; the import-first
        // flow defaults to off. Reset any value persisted under the old default.
        if (!mmkvManager.decodeSettingsBool(AppConfig.PREF_AUTO_BEST_DEFAULT_MIGRATED, false)) {
            mmkvManager.encodeSettings(AppConfig.PREF_AUTO_BEST_CONFIG, false)
            mmkvManager.encodeSettings(AppConfig.PREF_AUTO_BEST_DEFAULT_MIGRATED, true)
        }

        // Auto-best discovery is an opt-in feature now; the default flow is
        // importing configs and connecting to the selected one.
        if (mmkvManager.decodeSettingsBool(AppConfig.PREF_AUTO_BEST_CONFIG, false)) {
        ...
        if (mainViewModel.isRunningFlow.value) {
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                if (config != null && config.server != null && config.serverPort != null) {
                    ...
                    if (!ok && mmkvManager.decodeSettingsBool(AppConfig.PREF_AUTO_BEST_CONFIG, false)) {
    ...
    private fun handleMenuClick(id: Int) {
        when (id) {
            ...
            R.id.auto_best_config -> handleAutoBestAction()
    ...
    private fun onNavItemClick(id: Int) {
        when (id) {
            R.id.core_settings -> requestActivityLauncher.launch(Intent(this, SettingsActivity::class.java))
    ...
    fun refreshGroupTabTitles() {
        ...
        groupsState.value = groups.map { group ->
            if (group.id.isEmpty()) {
                group
            } else {
    ...
    private fun removeServer(guid: String, position: Int) {
        // Only the config of an active connection is protected; a selected but
        // disconnected config can be deleted (MmkvManager clears the selection).
        if (guid == mmkvManager.getSelectServer() && mainViewModel.isRunningFlow.value) {
    ...
    private fun handleAutoBestAction() {
        if (mainViewModel.isLoadingFlow.value) {
            autoBestConfigManager.stop()
            mainViewModel.setLoading(false)
            return
        }
    ...
    private fun setSelectServer(guid: String) {
        ...
        if (guid != selected) {
            ...
            if (mainViewModel.isRunningFlow.value) {
                ...
                // Picking a server manually cancels a running auto-best search
                autoBestConfigManager.stop()
                mainViewModel.setLoading(false)
            }
        }
    ...
    private fun importBatchConfig(server: String?) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                // Manual imports never target the auto-refreshed Free group; they
                // would be wiped by the next discovery run.
                val importSubId = mainViewModel.subscriptionId
                    .takeUnless { it == AppConfig.AUTO_BEST_SUBSCRIPTION_ID }
                    .orEmpty()
                val (count, countSub) = angConfigManager.importBatchConfig(server, importSubId, true)
                delay(500L)
                withContext(Dispatchers.Main) {
                    when {
                        count > 0 -> {
                            toastSuccess(getString(R.string.title_import_config_count, count))
                            openImportedGroup(server, countSub, importSubId)
                        }
                        countSub > 0 -> {
                            // Subscription stored but the fetch returned nothing usable
                            toastError("Subscription added, but no configs were received. Check the link or your network.")
                            openImportedGroup(server, countSub, importSubId)
                        }
    ...
    /**
     * After an import, focuses the Configs tab on the group that received the
     * new servers and refreshes their latency.
     */
    private fun openImportedGroup(importedText: String?, countSub: Int, importSubId: String) {
        val targetGroup = if (countSub > 0) {
            val subscriptions = mmkvManager.decodeSubscriptions()
            subscriptions.firstOrNull { it.subscription.url == importedText?.trim() }?.guid
                ?: subscriptions.maxByOrNull { it.subscription.addedTime }?.guid
                ?: ""
        } else {
            importSubId.ifEmpty { AppConfig.DEFAULT_SUBSCRIPTION_ID }
        }

        if (targetGroup.isNotEmpty()) {
            mainViewModel.subscriptionIdChanged(targetGroup)
        }
        refreshGroupTabTitles()
        mainTabIndex.intValue = 1 // Configs tab
