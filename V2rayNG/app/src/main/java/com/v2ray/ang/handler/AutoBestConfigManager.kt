...
import com.v2ray.ang.dto.LastWorkingConfig
import com.v2ray.ang.dto.ParseStats
    ...
    fun start(
        ...
        testingJob = CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
            ...
            try {
                ...
                if (lastWorking != null) {
                    // Source of truth is MMKV; the cached profile in lastWorking might be stale (e.g. edited by user)
                    val currentProfile = MmkvManager.decodeServerConfig(lastWorking.profile.guid)
                    val profileToVerify = currentProfile ?: lastWorking.profile

                    onProgress("Verifying last connection...")
                    val tcpTime = SpeedtestManager.socketConnectTime(
                        profileToVerify.server ?: lastWorking.host,
                        profileToVerify.serverPort?.toIntOrNull() ?: lastWorking.port,
                        2000
                    )
                    if (tcpTime > 0) {
                        ...
                        saveLastWorkingConfig(lastWorking.copy(profile = profileToVerify))

                        val guid = MmkvManager.encodeServerConfig(profileToVerify.guid, profileToVerify)
                ...
                val producerJob = launch {
                    val tcpSemaphore = Semaphore(if (lowMemory) 20 else 50)
                        ...
                        launch {
                            ...
                            tcpSemaphore.withPermit {
                                try {
                                    ...
                                    val tcpTime = SpeedtestManager.socketConnectTime(profile.server!!, profile.serverPort!!.toInt(), 1500)
